...
void wild_decrypt(char *infile_name, long long fsize, char *outfile_name, int key_length, int nonce_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, unsigned char *password) {
    ...
    free(msg);
    if (memcmp(mac, mac_verify, mac_length) == 0) {
        msg = (unsigned char *) malloc(fsize-mac_length-nonce_length-key_length-keywrap128_ivlen);
        unsigned char *nonce[nonce_length];
        fseek(infile, mac_length, SEEK_SET);
        unsigned char *kwnonce[keywrap128_ivlen];
        ...
        fread(keyprime, 1, key_length, infile);
        key_wrap_decrypt(keyprime, key_length, key, kwnonce);
        fread(msg, 1, (fsize - mac_length - nonce_length - key_length - keywrap128_ivlen), infile);
        fclose(infile);
        wild_crypt(msg, key, nonce, (fsize - mac_length - nonce_length - key_length-keywrap128_ivlen));
        outfile = fopen(outfile_name, "wb");
        fwrite(msg, 1, (fsize - mac_length - nonce_length - key_length - keywrap128_ivlen), outfile);
        fclose(outfile);
        free(msg);
    }
    else {
...
void ganja_decrypt(char *infile_name, long long fsize, char *outfile_name, int key_length, int nonce_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, unsigned char *password) {
    ...
    if (memcmp(mac, mac_verify, mac_length) == 0) {
        ...
        fread(msg, 1, (fsize - mac_length - nonce_length - key_length - keywrap256_ivlen), infile);
        fclose(infile);
        ganja_crypt(msg, keyprime, nonce, (fsize - mac_length - nonce_length - key_length-keywrap256_ivlen));
        outfile = fopen(outfile_name, "wb");
        fwrite(msg, 1, (fsize - mac_length - nonce_length - key_length - keywrap256_ivlen), outfile);
        fclose(outfile);
        free(msg);
...
void purple_decrypt(char *infile_name, long long fsize, char *outfile_name, int key_length, int nonce_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, unsigned char *password) {
    ...
    if (memcmp(mac, mac_verify, mac_length) == 0) {
        ...
        fread(msg, 1, (fsize - mac_length - nonce_length - key_length - keywrap256_ivlen), infile);
        fclose(infile);
        purple_crypt(msg, key, nonce, (fsize - mac_length - nonce_length - key_length-keywrap256_ivlen), key_length, nonce_length);
        outfile = fopen(outfile_name, "wb");
        fwrite(msg, 1, (fsize - mac_length - nonce_length - key_length - keywrap256_ivlen), outfile);
        fclose(outfile);
        free(msg);
...
void zanderCcbc_decrypt(char *infile_name, long long fsize, char *outfile_name, int key_length, int iv_length, int mac_length, int kdf_iterations, unsigned char * kdf_salt, unsigned char *password) {
    ...
    if (memcmp(mac, mac_verify, mac_length) == 0) {
        ...
        fread(msg, 1, (fsize - mac_length - iv_length - key_length - keywrap512_ivlen), infile);
        fclose(infile);
        int pad = zanderfishC_cbc_decrypt(msg, (fsize - mac_length - iv_length - key_length-keywrap512_ivlen),keyprime, key_length, iv, iv_length);
        outfile = fopen(outfile_name, "wb");
        fwrite(msg, 1, (fsize - mac_length - iv_length - key_length - keywrap512_ivlen - pad), outfile);
        fclose(outfile);
        free(msg);
